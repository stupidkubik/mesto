...
const popupCardFormValidator = new FormValidator(
  validationConfig,
  '.popup__form_card'
);
...
checkbox.addEventListener('change', () => {
  ...
  document.querySelector('.header__logo').classList.toggle('dark_logo');
  document.querySelector('.profile__name').classList.toggle('dark_text');
  document.querySelector('.profile__caption').classList.toggle('dark_text');
  document.querySelector('.profile__edit').classList.toggle('dark_text');
  document.querySelector('.profile__add-element').classList.toggle('dark_text');
  document
    .querySelectorAll('.element__name')
    .forEach((el) => el.classList.toggle('dark_element'));
});
...
const cardObj = {
  ...
  handleLike: async (notLiked, card) => {
    // Функция клика по лайкам
    try {
      if (notLiked) {
